...
define(
    ...
    function (objectUtils) {
        ...
        EditPersistableObjectsPolicy.prototype.allow = function (action, context) {
            ...
            var namespace = objectUtils.parseKeyString(domainObject.getId()).namespace;
            var provider = this.openmct.objects.getProvider(namespace);
            var key = action.getMetadata().key;
            var category = (context || {}).category;

            if (key === 'edit' ||
                (key === 'properties' && category === 'view-control') &&
                provider !== undefined) {
